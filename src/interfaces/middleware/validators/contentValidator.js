...
const validateCreate = [
  ...
  check("descripcion").optional().trim().not().isEmpty().withMessage("La descripción del contenido es requerida"),
  ...
  check("minutos_video").exists().trim().not().isEmpty().isInt({ min: 1 }).withMessage("Los minutos del vídeo debe ser un valor entero (aproximado))"),
...
const validateEdit = [
  ...
  check("minutos_video").optional().trim().not().isEmpty().isInt({ min: 1 }).withMessage("Los minutos del vídeo debe ser un valor entero (aproximado)"),
