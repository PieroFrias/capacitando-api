...
//         const categories = await categoriesService.getAllCategories(dataFilter, pageNumber, limit, rol);
...
//         const { id } = req.params;
//         const dataCategory = req.body;
//         const updatedCategory = await categoriesService.updateCategory(id, dataCategory);

//         if (updatedCategory) {
//             res.json({ message: "Categoria actualizado exitosamente" });
//         } else {
//             res.status(400).json({ error: "Categoria no encontrada o nombre de Categoria repetido" });
//         }
//     } catch (error) {
//         console.error(error);
//         res.status(500).json({ error: "Ocurrió un error en el servidor (controller - updateCategory)" });
//     }
// };
